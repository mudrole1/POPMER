...
(define (domain domain-tms-2-3-light)
  ...
  (:durative-action bake-ceramic1
      :parameters 
	(?p - piecetype1 ?k - kiln20)
      :duration (= ?duration 15)
      :condition
	 (over all (ready ?k))
      :effect (and
	 (at start (not (baked ?p)))
                   (at start (baking ?p))
                   (at end (not (baking ?p)))
                   (at end (baked ?p)))
  )
  ...
  (:durative-action make-structure
	...
	(?p1 - piece ?p2 - piece)
	 ...
	 (over all (baked ?p1))
                      ...
                      (over all (treated ?p2)))
      :effect
	 (at end (structured ?p1 ?p2))
  )

  (:durative-action bake-structure
      :parameters 
	(?p1 - piece ?p2 - piece ?k - kiln20)
      :duration (= ?duration 3)
      :condition
	 (and
	 (over all (ready ?k))
      ...
      :effect
	 (and
	 (at start (not (baked-structure ?p1 ?p2)))
